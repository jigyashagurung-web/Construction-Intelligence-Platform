...
- [ ] 6.4 Confirm a user from a different organisation cannot read another project's rollup, S-curve, or photos (RLS/storage policy check) — not testable with a self-signup account (all new signups land in the same hardcoded demo org); needs a second real org or a service-role script to test cross-org isolation.
