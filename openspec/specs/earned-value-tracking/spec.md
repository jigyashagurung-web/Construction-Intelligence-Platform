...
Computing and displaying Budget at Completion (BAC), Planned Value (PV), Earned Value (EV), and Actual Cost (AC) for a project, cost-weighted across BOQ-linked activities, along with the derived schedule and cost variance/performance metrics (SV, SPI, CV, CPI, EAC, VAC) that answer "are we on schedule?" and "are we on budget?".
